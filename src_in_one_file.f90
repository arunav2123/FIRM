...
!!$  call spline(psi_1d,fprime,npsi,2.d30,2.d30,y2_fprime) !prepare the second order derivative needed in the cubic spline interpolation
  ...
  contains
    subroutine t()
      ...
      if(myid==0) call draw_3d_tokamak()   ! Changed by Arunav 14th Jan
...
contains
  ...
  subroutine check_whether_particle_in_lcfs(r,z,phi,loss)
    ...
    call PNPOLY(r,z,x_lcfs,z_lcfs,np_lcfs,INOUT) !find out wheter the point (r,z) is within the lcfs  !Modified by Arunav 15th Jan
    !        if (inout.eq.1) then !within the LCFS
    if (inout.eq.-1 .or.inout.eq.0) then !the particle is out of the limiter
       write(*,*) '==>This particle hits the LCFS at (R,Z,phi)=', r,z,phi
       !if(myid==0) then 
       !   open(newunit=h,file='RZphi_LCFS_hits.txt', status='unknown', action='write', position='append')
       !   write(h,*) r, z, phi
       !   close(h)
       !end 
       loss=.true.
  ...
  subroutine check_whether_particle_in_limiter2(r,z,phi,particle_weight,loss,loss_weight)
    !revsed to sum the weight of the lossed markers
    use constants,only:p_
...
!!$  real(p_), dimension(:), allocatable :: thermalized_time_myid, lost_time_myid
  ...
  if(myid==0)  write(*,*) 'Physical Time Simulated (in seconds) =', dtao*tn*(kend-kstart)
  ...
  if(myid==0) call orbit_classification(particle_energy=65._p_) !energy in unit of keV   !Arunav 14th Jan
...
!     call normalize_guiding_center_variables(ninjection3,rg_add,zg_add,phig_add,mu_add,vpar_add)
  ...
  if(myid==0) write(*,'(A60,ES15.4)') [character(60):: 'Number of physical particles that a marker represents:'], weight0  ! Modifiesd 21st Jan
  ...
  if(myid==0) write(*,*) 'Source Power (MW)=', source_power
